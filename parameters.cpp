...
ClientParameters::ClientParameters(int argc, char *argv[]) : opt_description("Allowed options"), var_map() {
    try {
        ...
        if (var_map.count("help")) {
            ...
            Logger::print_error_and_exit("to little arguments");
            exit(1);
        }
    }
    catch (std::exception &e) {
        Logger::print_error_and_exit(e.what());
    }
}

void ClientParameters::initialize_options_description() {
    opt_description.add_options()
            ("gui-address,d", po::value<Address>(),
             "set gui address, arg as (nazwa hosta):(port) or (IPv4):(port) or (IPv6):(port)")
            ("help,h", "print help information")
            ("player-name,n", po::value<std::string>(), "set player's name")
            ("port,p", po::value<uint16_t>(), "set port number for receiving gui messages")
            ("server-address,s", po::value<Address>(),
